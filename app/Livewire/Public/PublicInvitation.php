...
class PublicInvitation extends Component
{
    ...
    public function mount(Invitation $invitation): void
    {
        ...
        $this->invitation = $invitation->load([
            'template',
            'galleries',
            'music',
            'digitalGifts',
            'guestbookEntries' => fn ($q) => $q->where('is_visible', true)->latest()->limit(20),
        ]);
    ...
    {
        $data      = $this->invitation->getInvitationData();
        $groom     = $data['groom_name'] ?? 'Mempelai';
        $bride     = $data['bride_name'] ?? 'Mempelai';
        $eventDate = $data['event_date'] ?? null;
        $location  = $data['location']   ?? '';

        $title       = "Undangan Pernikahan {$groom} & {$bride}";
        $description = $location
            ? "Dengan penuh kebahagiaan, kami mengundang Anda ke pernikahan {$groom} & {$bride}" . ($eventDate ? " pada " . \Carbon\Carbon::parse($eventDate)->translatedFormat('d F Y') : '') . " di {$location}."
            : "Dengan penuh kebahagiaan, kami mengundang Anda ke pernikahan {$groom} & {$bride}.";
