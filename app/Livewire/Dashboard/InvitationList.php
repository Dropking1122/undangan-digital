...
class InvitationList extends Component
{
    ...
    public function createInvitation(): void
    {
        ...
        $invitation = app(InvitationService::class)->create(auth()->user(), $template, [
            ...
            'invitation_data' => ['groom_name'=>$this->groomName,'bride_name'=>$this->brideName],
        ]);
    ...
    {
        return view('livewire.dashboard.invitation-list', [
            'invitations' => auth()->user()->invitations()->with('template')->latest()->get(),
            'templates' => Template::where('status','active')->with('category')->get(),
        ])->layout('layouts.app');
    }
}
